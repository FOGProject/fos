...
function display_output(
	...
	for (pName in partition_names) {
		printf(
            "%s : start=%10d, size=%10d, %s%2s",
            partitions[pName, "device"],
            partitions[pName, "start"],
            partitions[pName, "size"],
            type,
            partitions[pName, "type"]
        );
		if (label == "dos") {
			if(partitions[pName, "flags"] != "") {
				printf(
                    "%s",
                    partitions[pName, "flags"]
                );
			}
		} else if (label == "gpt") {
			if(partitions[pName, "uuid"] != "") {
				printf(
                    ", uuid=%s",
                    partitions[pName, "uuid"]
                );
			}
			if(partitions[pName, "name"] != "") {
				printf(
                    ", name=%s",
                    partitions[pName, "name"]
                );
			}
			if(partitions[pName, "attrs"] != "") {
				printf(
                    ", attrs=%s",
                    partitions[pName, "attrs"]
                );
			}
		} else {
			if(partitions[pName, "flags"] != "") {
	...
	for (pName in partition_names) {
		...
		if ((p_type == "5"
			...
			if((new_start < p_start + extended_margin
                || new_start >= p_start + p_size)
                || (new_start + new_size <= p_start + extended_margin
                || new_start + new_size > p_start + p_size)
            ) {
				return 1;
			}
		} else if ((new_type == "5"
			...
			if ((p_start < new_start + extended_margin
                || p_start >= new_start + new_size)
                || (p_start + p_size <= new_start + extended_margin
                || p_start + p_size > new_start + new_size)
            ) {
				return 1;
			}
		} else {
...
function fill_disk(
	...
	for (pName in partition_names) {
		...
		# + CHUNK_SIZE to allow for margin after each logical 
        # partition (required if 2 or more logical partitions exist)
		if (p_number >= 5) {
			original_fixed += CHUNK_SIZE;
		}
		if (p_size == 0) {
	...
	for (i in ordered_starts) {
		...
		# + CHUNK_SIZE to allow for margin after each
        # logical partition (required if 2 or more logical partitions exist)
		if (p_number >= 5) {
            curr_start += CHUNK_SIZE;
		}
	}
